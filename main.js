...
function sendWindowState(){
  ...
  mainWindow.webContents.send('nuvem-window-state', {maximized:mainWindow.isMaximized(),fullscreen:mainWindow.isFullScreen()});
}
function configureUpdater(){
  if(!app.isPackaged || updaterConfigured) return;
  updaterConfigured=true;autoUpdater.logger=log;autoUpdater.autoDownload=true;autoUpdater.autoInstallOnAppQuit=true;autoUpdater.allowDowngrade=false;
...
app.whenReady().then(async()=>{
  session.defaultSession.setPermissionRequestHandler((webContents,permission,callback)=>{const allowed=new Set(['media','fullscreen','notifications']);callback(allowed.has(permission))});
  if(session.defaultSession.setDisplayMediaRequestHandler){session.defaultSession.setDisplayMediaRequestHandler(async(request,callback)=>{try{const sources=await desktopCapturer.getSources({types:['screen','window'],thumbnailSize:{width:640,height:360},fetchWindowIcons:true});const chosen=sources.find(s=>s.id===pendingCaptureSourceId)||sources[0];activeCaptureSource=chosen?{id:chosen.id,name:chosen.name,type:String(chosen.id||'').startsWith('screen:')?'screen':'window'}:null;pendingCaptureSourceId=null;callback(chosen?{video:chosen,audio:'loopback'}:{})}catch(e){pendingCaptureSourceId=null;activeCaptureSource=null;callback({})}})}
  ipcMain.handle('nuvem-capture-sources',async()=>{const [screens,windows]=await Promise.all([desktopCapturer.getSources({types:['screen'],thumbnailSize:{width:640,height:360},fetchWindowIcons:true}),desktopCapturer.getSources({types:['window'],thumbnailSize:{width:640,height:360},fetchWindowIcons:true})]);const pack=(s,type)=>({id:s.id,name:s.name,type,thumbnail:s.thumbnail?.toDataURL?.()||'',icon:s.appIcon?.toDataURL?.()||''});return [...screens.map(s=>pack(s,'screen')),...windows.map(s=>pack(s,'window'))]});
  ipcMain.handle('nuvem-capture-select',(_,sourceId)=>{pendingCaptureSourceId=String(sourceId||'');return true});
  ipcMain.handle('nuvem-capture-state',()=>activeCaptureSource||{});ipcMain.handle('nuvem-copy-text',(_,text)=>{clipboard.writeText(String(text||''));return true});
  ...
  ipcMain.handle('nuvem-update-install',()=>{if(!app.isPackaged)return false;autoUpdater.quitAndInstall(true,true);return true});ipcMain.handle('nuvem-update-status',()=>({version:app.getVersion(),packaged:app.isPackaged}));
  createWindow();configureUpdater();app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow()});
});
